...
public class Honey implements Comparable<Honey> {
    ...
    public void computeConflicts() { // compute the number of conflicts to calculate fitness
        ...
        for (int i = 0; i < MAX_LENGTH; i++) {
            x = i;
            y = this.nectar[i]; // will result to no horizontal and vertical conflicts because it will result to
                                // diagonal

            // Check diagonals.
            for (int j = 0; j < 4; j++) { // because of dx and dy where there are 4 directions for diagonal searching
                                          // for conflicts
                tempx = x;
